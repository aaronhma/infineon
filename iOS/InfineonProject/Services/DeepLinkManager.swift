...
final class DeepLinkManager {
  ...
  /// Prefilled invite code from deep link
  var prefillInviteCode: String?
  ...
  private func handleInfineonScheme(_ url: URL) {
    ...
    switch host {
    case "add-vehicle":
      // Check if there's a code in the path: infineon://add-vehicle/ABC123
      let pathComponents = url.pathComponents.filter { $0 != "/" }
      if let code = pathComponents.first, !code.isEmpty {
        prefillInviteCode = String(code.uppercased().prefix(6))
      } else {
        prefillInviteCode = nil
      }
      showJoinVehicle = true
