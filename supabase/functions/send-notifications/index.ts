...
async function getVehicleUserTokens(
  ...
  // Step 1: Get all user_ids with access to this vehicle
  ...
  if (!accessData || accessData.length === 0) {
    console.log("No users have access to this vehicle");
    return [];
  }
...
async function handleWebhook(payload: WebhookPayload): Promise<Response> {
  ...
  // Handle face_detections webhook (alert on high intoxication or drowsiness)
  if (table === "face_detections" && record.vehicle_id) {
    const isDrowsy = record.is_drowsy;
    const isImpaired =
      record.intoxication_score && record.intoxication_score >= 3;

    if (!isDrowsy && !isImpaired) {
      return new Response(
        JSON.stringify({ skipped: true, reason: "No alert needed" }),
        {
    ...
    // Determine which preference key to check based on alert type
    const preferenceKey = isDrowsy ? "driver_drowsiness" : "drunk_driving";
    const tokens = await getVehicleUserTokens(record.vehicle_id, preferenceKey);
    ...
    let title = "Driver Alert";
    let body = "";

    if (isDrowsy) {
      title = "Drowsy Driver Detected";
      body = "The driver appears to be drowsy. Please check on them.";
    } else if (isImpaired) {
      title = "Impairment Alert";
      body = `Driver showing signs of impairment (score: ${record.intoxication_score}/6).`;
    }
    ...
    const requests: NotificationRequest[] = tokens.map((token) => ({
      ...
      thread_id: `vehicle_${record.vehicle_id}`,
      data: {
        vehicle_id: record.vehicle_id,
        detection_id: record.id,
        alert_type: isDrowsy ? "drowsy" : "impairment",
      },
    }));
